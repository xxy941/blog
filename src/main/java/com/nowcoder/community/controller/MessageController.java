...
public class MessageController implements CommunityConstant {
    ...
    public String getConversationIdNoticeList(Model model,@PathVariable("conversationId")String topic,Page page){
        ...
        /** 分页信息 */
        page.setLimit(5);
        page.setPath("/notice/" + topic + "/list");
        page.setRows(messageService.findNoticeCount(user.getId(),topic));

        List<Message> noticesList = messageService.findConversationIdNotices(user.getId(),topic,page.getOffset(), page.getLimit());
        ...
        for (Message notice : noticesList) {
            ...
            map.put("entityCommentType",0);
            if((int)data.get("entityType") == 2){
                Comment comment = commentService.findCommentById((int)data.get("entityId"));
                map.put("entityCommentType",comment.getEntityType());
            }
